...
/* USB Serial Number Descriptor Unit ID */
#define SERIAL_UNITID_DSC   0x03
#define UNITID_MAX_LEN      15
#define UNITID_DSC_LEN      ((2 * UNITID_MAX_LEN) + 2)
    // byte 0 = length in bytes = (2 * IDLen) + 2
    // byte 1 = DSC_STR (0x03)
    // byte 2 = UNITID[0] or 0x09 if no UnitID
    // byte 3 = 0         or 0x04 if no UnitID
    // byte 4 = UNITID[1]
    // byte 5 = 0
    // etc.
...
extern unsigned char unit_id_usb_buf[UNITID_DSC_LEN];
